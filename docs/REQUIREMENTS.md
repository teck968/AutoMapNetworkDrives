...
- **FR-11.** Credential prompting follows a **two-phase model** in manual mode:
  - **FR-11.1.** Phase 1 (discovery): scan all hosts, resolve hostnames, and attempt share enumeration using credentials already stored in Windows Credential Manager (FR-12). Hosts without stored credentials are recorded but not prompted-for yet.
    ...
    **Detect-and-prompt for silent-enum hosts**: a host's enum can succeed without the script ever explicitly authenticating — e.g. via Windows **workgroup pass-through** (the user's local Windows account name + password match an account on the remote host) or via **LSA credential cache** entries left over from earlier `net use \\HOST\IPC$ /user:... password` interactions in the same session. In both cases no entry gets written to Credential Manager. Pass-through is durable across reboots; LSA cache is not — it evicts at sign-out, leaving the persistent mapping with nothing to reconnect against, surfacing as a `Win32 85` ghost on next sign-in. The script can't distinguish the two cases, so when Phase 1 enum succeeds AND no `CRED_TYPE_DOMAIN_PASSWORD` credential exists for the host (per FR-11.5), the host is deferred to Phase 2 anyway to capture credentials for cross-reboot reliability. Skipped under `-Silent` (Phase 2 won't run) — silent-mode runs accept the pass-through-vs-cache ambiguity.
...
- **FR-39.** When invoked through the launcher (`Map-NetworkDrives.cmd`), if the launcher folder is a git working tree (i.e. has a `.git` subdirectory) and the network is reachable, the launcher attempts a fast-forward `git pull` from origin before starting the PowerShell script. Updates to `src/Map-NetworkDrives.ps1` take effect on the same run because the pull happens in the launcher (cmd) before PowerShell parses the script. **Updates to `Map-NetworkDrives.cmd` also take effect on the same run**: when the pull advances HEAD, the launcher re-executes itself in the same console (`cmd /c "%~f0" %* -NoUpdate`) so cmd.exe parses the new file content from a clean start, sidestepping cmd's mid-execution file-re-read behavior. Output from the original launcher and the re-exec appears in the same console window in chronological order; only one pause-at-end fires (the re-exec's).
